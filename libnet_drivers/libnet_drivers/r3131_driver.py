...
class r3131_driver:
    ...
    def get_sweep_time(self):
        return float(self.gpib_driver.query(b'ST?').decode().rstrip())
    ...
    def get_marker_frequency(self):
        return float(self.gpib_driver.query(b'MF?').decode().rstrip())
    def get_marker_level(self):
        return float(self.gpib_driver.query(b'ML?').decode().rstrip())
    def wait_for_opc(self):
        op_status_reg = self.gpib_driver.query(b'*ESR?').decode()
        print(op_status_reg)
...
if __name__ == "__main__":
    ...
    #print(r3131.gpib_driver.query(b'ST?').decode())
